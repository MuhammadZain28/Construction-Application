...
export class Records {
    ...
    static async save(name: string, wallet: string, amount: number, type: 'In' | 'Out', date: string, reason?: string): Promise<string> {
        ...
        await update(newRecordRef, {
            name: name,
            wallet: wallet,
            TotalAmount: type === 'In' ? amount : -amount,
        });
    ...
    static async getRecordsByID(recordId: string): Promise<Data[]> {
        const db = await Database();
        const recordsRef = ref(db, `records/${recordId}`);
        const snapshot = await get(recordsRef);
        const records: Data[] = [];
        console.log(snapshot);
        if (snapshot.exists()) {
...
export class Data {
    ...
    date: string;
    reason?: string;
    constructor(id: string, amount: number, type: 'In' | 'Out', date = new Date().toISOString().split('T')[0], reason?: string) {
        this.id = id;
        this.amount = amount;
        this.type = type;
        this.date = date;
