...
const transactionReasons = [
  ...
  const saveRecord = async (type: 'In' | 'Out') => {
    ...
    const amount = type === 'In' ? TotalAmount + Form.cash : TotalAmount - Form.cash;
    setTotalAmount(amount);
    const id = await Data.save(RecordId, Form.cash, type, Form.date.toISOString().substring(0, 10), amount, Form.reason);
    setRecord(m => [...m, { id: id, amount: Form.cash, type, date: Form.date.toISOString().substring(0, 10), reason: Form.reason }]);
    setState(prev => ({ ...prev, updateVisible: false }));
    setForm(prev => ({ ...prev, cash: 0, date: new Date(), reason: "Other", id: "" }));
    setFormType("");
  };
  const updateRecord = async (type: 'In' | 'Out') => {
    ...
    const amount = type === 'In' ? RemainingAmount + Form.cash : RemainingAmount - Form.cash;
    setTotalAmount(amount);
    await Data.updateData(RecordId, Form.id, Form.cash, Form.type, Form.date.toISOString().substring(0, 10), amount, Form.reason);
    setRecord(prev => prev.map(item => item.id === Form.id ? new Data(item.id, Form.cash, type, Form.date.toISOString().substring(0, 10), Form.reason) : item));
    setState(prev => ({ ...prev, updateVisible: false }));
    setForm(prev => ({ ...prev, cash: 0, date: new Date(), reason: "Other", id: "" }));
    setFormType("");
  };
  const deleteRecord = async () => {
    ...
    setTotalAmount(amount);
    await Data.deleteData(RecordId, Form.id, amount);
    setRecord(prev => prev.filter(item => item.id !== Form.id));
    setState(prev => ({ ...prev, updateVisible: false }));
    setForm(prev => ({ ...prev, cash: 0, date: new Date(), reason: "Other", id: "" }));
    setFormType("");
  };
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.body}>
        <View style={styles.row}>
          ...
          <TouchableOpacity style={[styles.row, { gap: 10, paddingBlock: 10, paddingInline: 20, backgroundColor: 'rgba(0, 35, 123, 1)', borderRadius: 50 }]} onPress={() => { setFormType('Record'); setState(prev => ({ ...prev, updateVisible: true })); setForm(prev => ({ ...prev, cash: 0, date: new Date(), reason: "Other", id: "" })); }}>
            ...
            <FlatList 
              ...
              renderItem={({ item }) => (
                <TouchableOpacity onPress={() => {
                  setForm(prev => ({ ...prev, id: item.id, cash: item.amount, date: new Date(item.date), reason: item.reason || 'Other', type: item.type }));
                  setFormType('Record');
                }}>
        ...
        <View style={{ gap: 10, marginTop: 10 }}>
          ...
          onLongPress={() => {setForm(prev => ({...prev, id: item.id})); setFormType('Delete'); setPrevCash(item.amount); }}
          onPress={() => {
                  setForm(prev => ({...prev, id: item.id, cash: item.amount, date: new Date(item.date), reason: item.reason || 'Other', type: item.type}));
          ...
          onLongPress={() => {setForm(prev => ({...prev, id: item.id})); setFormType('Delete'); setPrevCash(item.amount); setPrevType(item.type); }} 
          onPress={() => {
                  setForm(prev => ({...prev, id: item.id, cash: item.amount, date: new Date(item.date), reason: item.reason || 'Other', type: item.type}));
                  setPrevCash(item.amount);
                  setPrevType(item.type);
      ...
      <Modal
        ...
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0, 0, 0, 0.5)', gap: 30}}>
          ...
          <View style={styles.form}>
            ...
            <TextInput style={[styles.input, {outline: 'none'}]} value={Form.cash.toString()} onChangeText={ (e) => setForm(prev => ({...prev, cash: parseInt(e) || 0}))} keyboardType='numeric'/>
            ...
            <View style={[styles.input, {flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', zIndex: 10}]}>
              <TextInput style={[{color: 'rgb(0,0,0)', width: '100%', outline: 'none'}]} value={Form.reason} onChangeText={(e) => {setForm(prev => ({...prev, reason: e})); setState(prev => ({...prev, showReasons: true}))}}/>
              <TouchableOpacity>
                <Ionicons name='chevron-down' size={20} color={'rgb(0,0,0)'}/>
              </TouchableOpacity>
              { (Form.reason.length > 0 && state.showReasons) &&
                <FlatList
                      ...
                      renderItem={({ item }) => (
                        <TouchableOpacity onPress={() => {
                          ...
                          setState(prev => ({...prev, showReasons: false}));
            ...
            { !state.mobile ?
              ...
              }))} style={{ borderBottomColor: '#000', height: 40, borderBottomWidth: 1, borderInlineWidth: 0, borderTopWidth: 0, marginBottom: 20, fontFamily: 'Arial'}}/>
              :
              <View>
              ...
              onPress={() => setState(prev => ({ ...prev, showDate: !prev.showDate }))}>
