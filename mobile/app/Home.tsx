...
const Home: React.FC = () => {
  ...
  const [house, setHouse] = React.useState<House[]>([new House("All Houses", "All Houses", "")]);
  ...
  React.useEffect(() => {
    const fetchHouses = async () => {
      try {
        ...
        alert("Error fetching houses:"+ error);
      }
    }
  ...
  const addData = async() => {
    const ids = await Material.save(house[home], product, no, price, date.toLocaleDateString(), false);
    const data = new Material(ids, house[home], product, no, price, date.toLocaleDateString(), false);
  ...
  const handleUpdate = () => {
    setMaterial(prev =>
      prev.map(m =>
        m.id === productId
          ? new Material(m.id, house[home], product, no, price, date.toLocaleString(), m.used) // status = true
          : m
      )
    );
